...
"""
    ...
    def generate(self, output_path: Union[str, Path]) -> Path:
        ...
        try:
            ...
            violations = check_file(str(output_path))
            for v in violations:
                logger.warning(f"Brand violation: {v}")
        except ImportError:
            pass
...
"""
    ...
    def generate(self, output_path: Union[str, Path]) -> Path:
        ...
        # Write files
        (output_path / "app.py").write_text(self._generate_app_py())
        (streamlit_dir / "config.toml").write_text(self._generate_config_toml())
        (output_path / "requirements.txt").write_text(self._generate_requirements())

        # Create README
        (output_path / "README.md").write_text(f"""# {self.title}
...
class KDSReactApp:
    ...
    def generate(self, output_path: Union[str, Path]) -> Path:
        ...
        # Export data as JSON
        data_snapshot = self._data.snapshot()
        (output_path / "src" / "data" / "data.json").write_text(json.dumps(data_snapshot, indent=2))

        # Generate theme file
        theme_js = self.theme.to_react_theme()
        (output_path / "src" / "styles" / "theme.js").write_text(f"export const theme = {json.dumps(theme_js, indent=2)};")
        ...
        (output_path / "vite.config.js").write_text("""import { defineConfig } from 'vite'
...
""")
        ...
        # Generate index.html
        (output_path / "index.html").write_text(f"""<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
...
""")
        ...
        # Generate main.jsx
        (output_path / "src" / "main.jsx").write_text("""import React from 'react'
...
""")
        ...
        # Generate CSS
        (output_path / "src" / "styles" / "index.css").write_text(f"""
:root {{
...
""")
        ...
        (output_path / "src" / "App.jsx").write_text(f"""import React from 'react';
